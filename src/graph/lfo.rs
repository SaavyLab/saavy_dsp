use crate::{dsp::oscillator::OscillatorBlock, graph::node::{GraphNode, RenderCtx}};
...
mod tests {
  ...
  fn test_lfo_sine_output_range() {
    ...
    for &sample in &buffer {
      assert!(sample >= -1.0 && sample <= 1.0,
              "LFO sine sample {} out of range [-1.0, 1.0]", sample);
    }
  }
  ...
  fn test_lfo_triangle_output_range() {
    ...
    for &sample in &buffer {
      assert!(sample >= -1.0 && sample <= 1.0,
              "LFO triangle sample {} out of range [-1.0, 1.0]", sample);
    }
  }
  ...
  fn test_lfo_square_output_range() {
    ...
    for &sample in &buffer {
      assert!(sample >= -1.0 && sample <= 1.0,
              "LFO square sample {} out of range [-1.0, 1.0]", sample);
    }
  }
  ...
  fn test_lfo_saw_output_range() {
    ...
    for &sample in &buffer {
      assert!(sample >= -1.0 && sample <= 1.0,
              "LFO saw sample {} out of range [-1.0, 1.0]", sample);
    }
  }
  ...
  fn test_lfo_ignores_note_frequency() {
    ...
    for (i, (&s1, &s2)) in buffer1.iter().zip(&buffer2).enumerate() {
      assert!((s1 - s2).abs() < 1e-6,
              "LFO output differs at sample {}: {} vs {}", i, s1, s2);
    }
  }
}